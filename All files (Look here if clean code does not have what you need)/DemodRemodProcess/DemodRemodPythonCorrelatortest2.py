...
Interpolation=100
offsetvalue=int(8400*8)
sns.set_theme()
result_array=np.zeros((6,100))
for i in np.arange(1.0,11.0,1):    
    for j in range(0,1):
        ...
        ReceivedSignal=np.fromfile('D:\GNURADIO_THESIS_MASTER\SimulationTESTDEMODREMOD\Signal2', dtype=np.complex64,count=countvalue, sep='', offset=offsetvalue) 
        
        SentSignal=np.fromfile('D:\GNURADIO_THESIS_MASTER\SimulationTESTDEMODREMOD\Signal1', dtype=np.complex64, count=countvalue, sep='', offset=offsetvalue) 
