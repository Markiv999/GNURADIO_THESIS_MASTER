...
#Add def main when you want to run this code as a function within another script.
#def main():
#%% Start: Set initial Params
print("Correlator was Called")
bitrate=9600
sample_rate=500000
sps=int(sample_rate/bitrate)

#%% Set Correlation Params : To simplify the setting of params, calculate pearsons correlation coefficient and set a normalized threshold for correlation value (currently threshold)

#########################################################
#Set this to double the expected delay
min_delay=1000
...
cut_off_limit_correlation=0.004
...
#Peak breaker should be bigger than the width of a peak, exists to avoid double and wrong inputs from a peak
delaypeakbreaker=75000

#Ignore original length now, max delay is set a little higher than expected delay (I used 1.5)
original_length=0

max_delay_allowed=50000+original_length
##########################################################

#%% Read the raw transmission and reception streams (for length determination) and demodded streams. Finally read reference marker stream
...
#%% Ignore, residuals code from early testing
"""
...
#%% Check for marker sequence in demodulated reception stream
HeaderSignal=np.pad(HeaderSignal,(0,DemodSignalReceived.shape[0]-HeaderSignal.shape[0]))

#Pad by 2 to avoid cyclic convolution
#Here optionally, you can pad to next power of 2 to improve the speed of fft.
...
# Invert demodulated stream if the peak is negative for visualization purposes
...
# Plot of marker at the location detected in reception stream
...
#%% Calculating Pseudo Range 1 - Sent Signal Location


#Signal Length is sps*24*8 of data sandwiched between sps*50*8 of Pilot and sps*50*8 of tail
# Command signal properties of transmission stream.
Signal_length=sps*24*8  
PT_length=sps*700*8
offset=Signal_length+2*PT_length
i=0
SentSignal_location=[]
#Find out transmisson location (time) of command signals
...
#%% Calculating Pseudo Range 2 - Received Signal Location
p=0
delay_peak_previous=0

#Find out location of Received markers from Correlation_values_3 matrix
...
#%% Calculating Pseudo Range 3 - Find Pseudo Range
...
#%% Experimental Code to extract the satellite on board timing information attached behind received markers - Not working
import struct
