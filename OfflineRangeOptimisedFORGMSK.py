...
sentvalues=np.fromfile('D:\\GNURADIO_THESIS_MASTER\\CommandSignals\\CommandSignal1', dtype=np.ubyte, count=vectorsize, sep='', offset=0)   
...
while flag==0:
    ...
    #no need already in bytes: divide by 8 to give offset in bytes
    receivedvalues=np.fromfile( 'D:\\GNURADIO_THESIS_MASTER\\GMSKSIGNALS\\GMSKtest1' , dtype=np.ubyte, count=vectorsize, offset=(initial_offset+  int(vectorsize)*(overall_counter)))
